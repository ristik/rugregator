...
use crate::storage::{BlockInfo, FinalizedRecord, PendingRound, RecordInfo, RecoveredState, Store, WalRecord, WalStore};
...
impl WalStore for RocksDbStore {
    fn write_pending_round(&self, round: &PendingRound) -> anyhow::Result<()> {
        ...
        self.db.put_cf(&cf, KEY_PENDING, encode_pending_round(round))?;
        Ok(())
    }
...
fn decode_pending_round(buf: &[u8]) -> anyhow::Result<PendingRound> {
    ...
    let zk_proof     = if zk_raw.is_empty() { None } else { Some(zk_raw) };
    ...
    for _ in 0..count {
        ...
        inserted.push(WalRecord { state_id, predicate_cbor, source_state_hash, transaction_hash, witness });
    }
    Ok(PendingRound { block_number, prev_root, new_root, zk_proof, new_leaves, state_size, inserted })
}

fn write_opt_hash(buf: &mut Vec<u8>, h: Option<&[u8; 32]>) {
    match h {
        Some(hash) => { buf.push(1); buf.extend_from_slice(hash); }
        None       => { buf.push(0); }
    }
}

fn read_opt_hash(buf: &[u8], p: &mut usize) -> anyhow::Result<Option<[u8; 32]>> {
    if buf.len() <= *p { anyhow::bail!("truncated opt-hash flag"); }
    let flag = buf[*p];
    *p += 1;
    if flag == 0 {
        return Ok(None);
    }
    if buf.len() < *p + 32 { anyhow::bail!("truncated opt-hash value"); }
...
fn read_u32(buf: &[u8], p: &mut usize) -> anyhow::Result<u32> {
    if buf.len() < *p + 4 { anyhow::bail!("truncated u32"); }
