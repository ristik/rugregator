...
4. **Timeout**: a request may only be inserted in a round whose reference time is strictly below its deadline. `CertificationData` version 2 always carries the deadline slot. An explicit deadline is encoded as an unsigned integer and committed by the transaction hash; an omitted deadline is encoded as CBOR null and the service assigns `reference_time + AGGREGATOR_DEFAULT_REQUEST_TTL_SECS`. The assigned deadline is service metadata and never enters the transaction hash, so a requester that omits it needs no clock of its own. Admission fails an already-expired request early; the authoritative check runs where the leaf is materialised, against that round's pinned reference time.
